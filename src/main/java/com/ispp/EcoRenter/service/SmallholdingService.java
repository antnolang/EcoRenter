...
public class SmallholdingService {
    ...
    public Smallholding save(final Smallholding smallholding, final List<MultipartFile> file) {
	Assert.notNull(smallholding, "La parcela debe existir");
	Assert.isTrue(smallholding.getOwner().equals(this.ownerService.findByPrincipal()),
		"El propietario de la parcela no corresponde con el usuario autenticado");
	...
	if (smallholding.getId() != 0 && file.get(0).getOriginalFilename().equals("")) {
    ...
    public Smallholding save(final Smallholding smallholding) {
	Assert.notNull(smallholding, "La parcela debe existir");
	Assert.isTrue(smallholding.getOwner().equals(this.ownerService.findByPrincipal()),
		"El propietario de la parcela no corresponde con el usuario autenticado");
    ...
    public void delete(final Smallholding smallholding) {

	this.smallholdingRepository.delete(smallholding);
    }

    public Smallholding rent(final Smallholding smallholding) {
	Assert.notNull(smallholding, "La parcela debe existir");
    ...
    public void deactivate(final Smallholding smallholding) {
    ...
    public void activate(final Smallholding smallholding) {
	Assert.notNull(smallholding, "La parcela no debe ser nula");
	Assert.isTrue(smallholding.getOwner().equals(this.ownerService.findByPrincipal()),
		"El propietario de la parcela no corresponde con el usuario autenticado");
	Assert.isTrue(smallholding.getStatus().equals("NO ALQUILADA"), "No se puede editar una parcela ya alquilada");
    ...
    public Smallholding findOne(final int smallholdingId) {
    ...
    @Transactional(value = TxType.NEVER) // Si se quita, al hacer display de parcela peta una clase interna de spring
					 // debido a un bug.
    public Smallholding findOneToDisplay(final int smallholdingId) {
	Smallholding result;
	Actor principal;
	...
	/*
	 * If principal is a renter or unauthenticated, he/she displays availables and
	 * not rented smallholdings only.
	 * 
	 * For the other side, if principal is an owner, he/she displays availables
	 * smallholdings unless it's his/her own so displays unavailables too.
	 */
	if (principal == null) {
	    Assert.isTrue(result.isAvailable() && result.getStatus().equals("NO ALQUILADA"),
		    "La parcela no se puede mostrar");
	} else if (principal != null && principal instanceof Renter) {
	    /*
	     * El arrendatario solo puede ver la parcela en caso de que no esté alquilada y
	     * su estado sea disponible o bien, en caso de que esté alquilada por el
	     * actualmente
	     */
	    isRentedByRenter = this.isSmallholdingRentedByRenter(principal.getId(), smallholdingId);
	    Assert.isTrue(result.isAvailable() && result.getStatus().equals("NO ALQUILADA") || isRentedByRenter,
		    "La parcela no se puede mostrar");
	} else if (principal != null && principal instanceof Owner) {
	    /*
	     * El propietario solo puede ver la parcela en caso de que no esté alquilada y
	     * su estado sea disponible o bien, en caso de que sea su parcela
	     */
	    Assert.isTrue(result.isAvailable() && result.getStatus().equals("NO ALQUILADA")
		    || result.getOwner().equals(principal), "La parcela no se puede mostrar");
	}
    ...
    public Smallholding findOneToEdit(final int smallholdingId) {
	Smallholding result;

	result = this.findOne(smallholdingId);
	Assert.isTrue(result.getOwner().equals(this.ownerService.findByPrincipal()),
		"El propietario de la parcela no corresponde con el usuario autenticado");
	Assert.isTrue(result.getStatus().equals("NO ALQUILADA"),
		"La parcela no se puede editar debido a que se encuentra en estado distinto a NO ALQUILADA");
    ...
    public Page<Smallholding> findPaginated(final Pageable pageable, final Collection<Smallholding> smallholdings) {
	...
	Page<Smallholding> smPage = new PageImpl<Smallholding>(list, PageRequest.of(currentPage, pageSize),
		smaList.size());

	return smPage;
    }

    public Collection<Smallholding> findSmallholdingsByOwnerId(final int ownerId) {
	Collection<Smallholding> result;
    ...
    public Boolean isSmallholdingRentedByRenter(final int renterId, final int smallholdingId) {
	Boolean result;
    ...
    public Collection<Smallholding> findSmallholdingsRentedByOwnerId(final int ownerId) {
	Collection<Smallholding> result;
    ...
    public Collection<Smallholding> findSmallholdingsByActiveRentOut(final int renterId) {
    ...
    public Collection<Smallholding> findSmallholdingsByKeyword(final String keyword) {
	Collection<Smallholding> result;
    ...
    public List<String> getGeoData(final Collection<Smallholding> smallholdings) {
