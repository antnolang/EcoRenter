...
public interface SmallholdingRepository extends JpaRepository<Smallholding,Integer> {
    ...
    @Query("select distinct s from Smallholding s where (s.isAvailable = true and s.status = 'NO ALQUILADA') and ((s.province like concat('%', concat(?1, '%'))) or (s.farmingType like concat('%', concat(?1, '%'))) or (CAST(s.size as text) like ?1) or (CAST(s.price as text) like ?1))")
