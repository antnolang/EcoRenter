...
public interface SmallholdingRepository extends JpaRepository<Smallholding, Integer> {
    ...
    @Query("select sh from Smallholding sh where sh.isAvailable=true and sh.isArgumented=false and sh.status = 'NO ALQUILADA' order by sh.owner.accumulatedMonths desc")
    ...
    @Query("select distinct ro.smallholding from RentOut ro where ro.renter.id = ?1")
    Collection<Smallholding> findSmallholdingsByRenterId(int renterId);
